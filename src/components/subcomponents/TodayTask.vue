	...
	<ul>
		<li v-for="task in tasks" v-if="task.isShow" v-bind:class="{ done: task.isDone}" v-bind:title="task.time">
			...
			<div class="handle-div" >
				<span class="comment" @click="addComment(task)" title="添加备注">C</span>
				<span class="del" @click="delTask(task)" title="删除">X</span>
			</div>
			<comment v-bind:task="task" v-show="task.showComment" v-on:fromComment="receiveComment"></comment>
		</li>
<!-- 		<li>
			<div class="circle">
			</div>
...
<script>
	...
	export default {
		...
		data () {
			return {
				nowInput: true,
				newTask:{ taskId:'', content: '', time: '', isDone: false, isShow: true, comment: '', showComment: false},
				tasks:[],
				// showComment: false
			}
		},
		...
		methods: {
			...
			// 接受子组件 comment 的值
			receiveComment (data) {
				data.obj.showComment = data.value
			}
		},
		mounted() {
